...
test('test', async ({ page }) => {
  ...
  await page.getByRole('row', { name: 'QA Testing (2) Just submitted' }).getByRole('button').click();
  ...
  await page.getByRole('button', { name: 'Yes, Approve & Publish' }).click();
  await page.getByRole('cell', { name: 'QA Testing (2)' }).click();
});